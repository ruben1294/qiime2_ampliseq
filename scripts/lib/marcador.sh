...
# -----------------------------------------------------------------------------
#  Resuelve qué marcador se analiza: ITS (hongos) o 16S (procariotas). A partir
#  de esa decisión elige el archivo de parámetros (YAML) que recibe Nextflow.
#  Lo llaman los scripts 02 y 03.
# =============================================================================
...
seleccionar_marcador() {
    ...
    case "$MARCADOR" in
        its|16s) : ;;
        *) log_error "MARCADOR no válido: '$MARCADOR' (usa its o 16s)"; exit 1 ;;
    esac

    # Archivo de parámetros según el marcador
    [ "$MARCADOR" = "its" ] && CONFIG_MARCADOR="$CONFIG_ITS" || CONFIG_MARCADOR="$CONFIG_16S"
